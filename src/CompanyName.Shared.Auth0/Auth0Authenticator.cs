...
namespace CompanyName.Shared.Auth0
{
    ...
    public static class Auth0AuthenticatorExtensions
    {
        ...
        /// <param name="request">The HTTP request.</param>
        /// <param name="cancellationToken">An optional cancellation token.</param>
        public static Task<(ClaimsPrincipal User, SecurityToken ValidatedToken)> AuthenticateAsync(this Auth0Authenticator @this, HttpRequest request, CancellationToken cancellationToken = new CancellationToken())
        {
            var header = request.Headers.ContainsKey("Authorization") ? request.Headers["Authorization"].FirstOrDefault() : null;
            if (string.IsNullOrWhiteSpace(header))
            {
                throw new InvalidOperationException("Authorization header missing.");
            }
            return @this.AuthenticateAsync(header, cancellationToken);
        }
    }
}
